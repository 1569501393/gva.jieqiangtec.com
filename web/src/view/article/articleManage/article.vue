<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="自增id" prop="id" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="text" icon="el-icon-edit" size="small" class="table-button" @click="updateArticle(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" label-width="80px">
        <el-form-item label="自增id:">
          <el-input v-model.number="formData.id" clearable placeholder="请输入" />
        </el-form-item>
      </el-form>
      <template #footer>
...
export default {
  ...
  // 条件搜索前端看此方法
    ...
    async updateArticle(row) {
      ...
      this.type = 'update'
      if (res.code === 0) {
        this.formData = res.data.rearticle
    ...
    async enterDialog() {
      ...
      switch (this.type) {
        case 'create':
          ...
          break
        case 'update':
          res = await updateArticle(this.formData)
          break
        default:
          res = await createArticle(this.formData)
          break
